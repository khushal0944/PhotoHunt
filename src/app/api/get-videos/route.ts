...
export async function GET(request: Request): Promise<Response> {
    ...
    try {
        const result: ResultDataType = await axios.get(`${api_url.replace("/v1", "/videos")}/search/`, {
            headers: {
        ...
        return NextResponse.json({data: result.videos, totalPages: Math.ceil(result.total_results/result.per_page)}, {status: 200})
    } catch (error) {
        console.error(error);
